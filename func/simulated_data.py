...
def simulate_ddm_trials(v,
    ...
    for _ in range(n_trials):
        ...
        choice = int(
            x
            > 0)  # 1 = upper boundary (e.g., "yes"), 0 = lower boundary ("no")
        rts.append(rt)
...
# Parameters
wang_params = {
    "positive_high_dominance": {
        "v": 1.0,
        "t0": 0.30
    },
    "positive_high_affiliation": {
        "v": 1.0,
        "t0": 0.30
    },
    "negative_low_dominance": {
        "v": -0.6,
        "t0": 0.36
    },
    "negative_low_affiliation": {
        "v": -0.6,
        "t0": 0.36
    }
}
...
for cond, params in wang_params.items():
    df = simulate_ddm_trials(v=params["v"],
                             t0=params["t0"],
                             n_trials=condition_counts[cond])
    df["condition"] = cond
    df_all.append(df)
