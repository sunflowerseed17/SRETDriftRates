...
df["Circumplex"] = (
    df["Circumplex"].astype(str).str.strip().str.lower().str.replace("  ", " ", regex=False)
)
df["Circumplex_no_space"] = df["Circumplex"].str.replace(" ", "", regex=False)

# Keep the four cells used in your R code
keep_mask = (
...
df["C"] = df["Circumplex2"].map({"affiliation": -1, "dominance": 1}).astype(int)
...
# 2) Sanity checks
# -------------------------------
assert set(df["response"].unique()).issubset({0, 1}), "response must be 0/1 ints"
assert df["V"].nunique() == 2 and df["C"].nunique() == 2, "V and C must vary and be ±1"
...
model = hddm.HDDMRegressor(
    data,
    reg_spec,
    group_only_regressors=False,   # <-- allow random slopes
    include=["v", "a", "t", "z"],
    p_outlier=0.05
)
...
# 4) Trace helpers
# -------------------------------
# Your HDDM version returns a single DataFrame with all traces
...
# -------------------------------
# 5) Group-level fixed effects (still useful to report)
# -------------------------------
...
# -------------------------------
# 6) Collect SUBJECT-SPECIFIC coefficients
# -------------------------------
...
# -------------------------------
# 7) Compute per-subject drift per condition USING SUBJECT COEFS
# -------------------------------
conditions = {
...
for subj in subjects:
    if not ({subj}.issubset(b0_subj.keys()) and
            {subj}.issubset(bV_subj.keys()) and
            {subj}.issubset(bC_subj.keys()) and
            {subj}.issubset(bVC_subj.keys())):
    ...
    for name, (v_code, c_code) in conditions.items():
        ...
        rows.append({"Subject": subj, "Condition": name, "Drift": float(drift)})
...
# -------------------------------
# 8) Merge symptoms & save
# -------------------------------
symptom_cols = ["LSAS", "SPIN", "FPES", "BFNE", "BDI", "RSES", "STAI-S", "STAI-T"]
symptoms = (
    df[["subj_idx"] + [c for c in symptom_cols if c in df.columns]]
    .drop_duplicates()
    .rename(columns={"subj_idx": "Subject"})
)
